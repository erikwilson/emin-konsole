...
class DeepLearnBase {
  ...
  create({m,n}) {
    ...
    const output = graph.layers.dense('layerOut', input, outSize, null, true)//, scale2)//, scale)
  ...
  async learn() {
    ...
    // )
    const testNum = 0
    const result = await this.run(this.net,inputs[testNum]).getValues()
    console.log(inputs[testNum],moves[testNum])
    console.log({result})
  }
